...
if __name__ == "__main__":
    ...
    for c_round in tqdm(range(args.n_rounds), desc="Training Progress", unit="round"):
        print(f"\nRound {c_round + 1}/{args.n_rounds}")
        
        aggregator.mix()

        if hasattr(aggregator, "get_training_stats"):
            stats = aggregator.get_training_stats()
            print(f"Stats for round {c_round + 1}: {stats}")
        else:
            print("Aggregator does not provide training stats. Skipping detailed log.")

        if (c_round % args.log_freq) == (args.log_freq - 1):
            aggregator.write_logs()
            print(f"Logs written for round {c_round + 1}")
